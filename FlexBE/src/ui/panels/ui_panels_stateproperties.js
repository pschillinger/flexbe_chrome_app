UI.Panels.StateProperties = new (function() {
	...
	var addHoverDocumentation = function(el, type, name, state_class) {
		var def = Statelib.getFromLib(state_class);
		var doc = undefined;
		switch (type) {
	...
	var addAutocomplete = function(el, state_class, mode, state) {
		...
		if (state_class != undefined) {
			var class_vars = Statelib.getFromLib(state_class).getClassVariables();
	...
	var displayPropertiesForBehavior = function(state) {
		...
		document.getElementById("label_prop_be_desc").innerText = Behaviorlib.getByName(state.getBehaviorName()).getBehaviorDesc();
