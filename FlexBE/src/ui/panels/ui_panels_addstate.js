UI.Panels.AddState = new (function() {
	...
	var displayStateClasses = function(class_list) {
		...
		for (var i=0; i<class_list.length; ++i) {
			state_def = Statelib.getFromLib(class_list[i]);
	...
	this.show = function() {
		statelib = Statelib.getClassList();
		displayStatelib();
	...
	this.addStateConfirmClicked = function() {
		...
		var state_def = Statelib.getFromLib(state_class);
		...
		ActivityTracer.addActivity(ActivityTracer.ACT_STATE_ADD,
			...
			function() {
				...
				var redo_state = new State(state_name, Statelib.getFromLib(state_class));
