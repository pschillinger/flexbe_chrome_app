LibParser = new (function() {
	...
	var parseState = function(content, import_path) {
		// Patterns
		...
		var state_doc = new Documentation(state_desc);
	...
	this.restoreFolderList = function(folder_list, display_callback) {
		lib_folders = folder_list;
		this.parseLibFolders();
		for (var i = 0; i < lib_folders.length; i++) {
			display_callback(lib_folders[i]);
		};
	}
